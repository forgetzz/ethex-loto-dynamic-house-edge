...
function placeBet(bytes22 params) external payable {
    ...
    uint256 houseEdgeFee = msg.value * dynamicHouseEdge / 100;
    ...
    require(
        msg.value *
            (100 - JACKPOT_PERCENT - dynamicHouseEdge) *
            (coefficient * 8 - 15 * markedCount) <=
            9000 ether * markedCount
    );

    require(
        msg.value *
            (800 * coefficient -
                (JACKPOT_PERCENT + dynamicHouseEdge) *
                (coefficient * 8 + 15 * markedCount)) <=
            1500 * markedCount * (address(this).balance - holdBalance)
    );
    ...
    function settleBets() external {
        ...
        for (; i < length; i++) {
            if (blockNumberQueue[first] >= block.number) {
                ...
                (coefficient, markedCount, holdAmount) = getHold(
                    bet.amount,
                    bet.bet
                );
                holdBalance -= holdAmount;
                ...
                if (bet.blockNumber < block.number - 256) {
                    ...
                    for (uint8 j = 0; j < bet.bet.length; j++) {
                        ...
                        bytes1 field;
                        if (j % 2 == 0) field = blockHash[29 + j / 2] >> 4;
                        else field = blockHash[29 + j / 2] & 0x0F;
                        if (bet.bet[j] < 0x10) {
                            if (field == bet.bet[j]) {
                                matchesCount++;
                                coefficient += 30;
                    ...
                    uint256 payoutAmount = (bet.amount * coefficient) /
                        (PRECISION * 15 * markedCount);
        ...
        for (i = 0; i < length; i++) {
            if (
                ...
                !transactions[i].gamer.send(transactions[i].amount)
            )
                (new DeliverFunds).value(transactions[i].amount)(
                    transactions[i].gamer
                );
            if (superprizes[i].id != 0) {
                EthexSuperprize(superprizeAddress).initSuperprize(
                    transactions[i].gamer,
                    superprizes[i].id
                );
                EthexJackpot(jackpotAddress).paySuperprize(
                    transactions[i].gamer
                );
                if (
                    superprizes[i].amount > 0 &&
                    !transactions[i].gamer.send(superprizes[i].amount)
                )
                    (new DeliverFunds).value(superprizes[i].amount)(
                        transactions[i].gamer
                    );
            }
        }
    ...
    function getHold(
        ...
        internal
        pure
        returns (uint256 coefficient, uint8 markedCount, uint256 holdAmount)
    {
